...
# Correção da criação dos quadrantes
df['Quadrante'] = np.select(
    [
        df['Líder'] == 'Jamerson Igor',
        df['Líder'] == 'João Belo',
        df['Líder'] == 'Leilson Alves',
        df['Líder'] == 'Maxsuel Conceição',
...
#st.write(f"👑 Mecânico que mais fez **Moto Cliente**", f"{qtd_cliente} ({top_cliente}) - Líder: `{mec_lider_dict.get(top_cliente, 'N/A')}`")
#st.write(f"👶 Mecânico que menos fez **Cliente**", f"{qtd_menos_cliente} ({menos_cliente}) - Líder: `{mec_lider_dict.get(menos_cliente, 'N/A')}`")
